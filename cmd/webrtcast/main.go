...
// connectTimeout bounds how long Connect waits for ICE gathering before giving
// up on a viewer — keeps the signalling loop responsive on flaky links.
...
func main() {
	...
	// New takes a factory: the broadcaster builds a fresh source for each run
	// (first viewer, or auto-restart after a crash), started lazily so nothing
	// is encoded until someone actually connects.
	//
	...
	// with a synthetic pattern instead, return webrtcast.NewFFmpegSource() here.
	newSource := func() webrtcast.VideoSource {
		return webrtcast.NewPiCameraSource()
	}
	caster := webrtcast.New(newSource,
		webrtcast.OnSourceError(func(err error) {
			slog.Error("video source error", "err", err)
		}),
	)
	defer caster.Close()
	...
	for {
		...
		answer, err := caster.Connect(ctx, sig.SDP)
		cancel()
		if err != nil {
